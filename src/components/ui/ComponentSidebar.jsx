 ...
 * 
 * Features:
 * - Fixed position on the left side (240px width)
...
const ComponentSidebar = ({ 
  ...
  // Base button styles for sidebar buttons
  const buttonClass = `
    w-full text-left px-4 py-3 mx-2 my-1 rounded-md 
  ...
  return (
    <aside className="fixed left-0 top-0 h-screen w-60 bg-notebook-red text-white shadow-lg z-40 flex flex-col">
      {/* Header Section */}
      <div className="p-4 border-b border-notebook-red-dark">
        <h2 className="text-sm font-semibold uppercase tracking-wide text-red-100">
          Components
        </h2>
        <p className="text-xs text-white opacity-70 mt-1">
          Add tools to your notebook
        </p>
      </div>
      ...
      <div className="p-4 border-t border-notebook-red-dark">
        {/* Save Status Indicator */}
        <div className="text-xs text-white opacity-70 text-center">
          <p>Notebook tools ready</p>
        </div>
      </div>
    </aside>
  )
